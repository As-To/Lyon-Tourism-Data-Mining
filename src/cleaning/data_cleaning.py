...
def apply_all_filters(df: pd.DataFrame, apply_buggy: bool = False) -> pd.DataFrame:
    ...
    #6) Remove columns starting with 'Unnamed'
...
def main():
    ...
    print(f"Lignes avant filtre géographique Lyon : {before}")
    print(f"Lignes après filtre Lyon géographique : {after}")
    ...
    after_cleaned = len(cleaned)
    print(f"Lignes avant filtrage des doublons: {before}")
    print(f"Lignes après filtrage des doublons: {after_cleaned}")
    print(f"Nombre de lignes supprimées : {before - after_cleaned}")
    #print(cleaned.head())
    ...
    print(f"Lignes avant filtrage des dates incohérentes: {before}")
    print(f"Lignes après filtrage des dates incohérentes: {after_cleaned_inconsistent}")
    print(f"Nombre de lignes supprimées : {before - after_cleaned_inconsistent}")
    ...
    print(f"Lignes avant filtrage des lignes buggées: {before}")
    print(f"Lignes après filtrage des lignes buggées: {after_cleaned_buggy}")
    print(f"Nombre de lignes supprimées : {before - after_cleaned_buggy}")
    ...
    print(f"Lignes avant suppression des colonnes d'upload: {before}")
    print(f"Lignes après suppression des colonnes d'upload: {after_cleaned_final}")
    print(f"Nombre de lignes supprimées : {before - after_cleaned_final}")
...
#    print(data_lyon_cleaned.head())
    ...
    before_lyon = len(data)
    data_lyon_cleaned = apply_all_filters(data_lyon, apply_buggy=True)
    after_lyon = len(data_lyon_cleaned)
    print("Application de toutes les étapes de nettoyage sur les données de Lyon")
    print(f"Lignes avant nettoyage complet des données de Lyon: {before_lyon}")
